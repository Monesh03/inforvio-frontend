    ...
    <div className="bg-white flex flex-col p-8" style={{ minHeight: '100vh' }}>
      ...
      {applicants.length > 0 && (
        <div style={{ borderBottom: '1px solid #e5e7eb' }}>
          <div className="flex items-center px-8 pt-5 gap-4">
            {applicants.map((a, i) => (
              <div key={a.applicantId} className="flex items-center gap-2"
                style={{ borderBottom: activeIndex === i ? '2px solid #3b82f6' : '2px solid transparent', marginBottom: '-1px' }}>
                <button onClick={() => switchApplicant(i)}
                  className="text-base font-semibold pb-2 transition-colors"
                  style={{ color: activeIndex === i ? '#3b82f6' : '#9ca3af' }}>
                  {a.name}
                </button>
                <button onClick={() => handleDeleteApplicant(a.applicantId)}
                  className="flex items-center justify-center w-10 h-10 rounded-lg text-white mb-2"
          ...
          <div className="flex flex-col gap-6" style={{ minWidth: 160 }}>
            {activeApplicant.documents.map((doc) => (
              <button
                ...
                className="px-5 py-3 rounded-lg text-base font-semibold text-white transition-all text-left"
                style={{
            ...
            <button onClick={() => setShowAddDoc(true)}
              className="flex items-center gap-2 px-5 py-3 rounded-lg text-base font-semibold text-white"
              style={{ backgroundColor: '#3b82f6', boxShadow: '0 0 0 2px #93c5fd', minWidth: 140 }}>
              + Add
            </button>
          </div>
